      ...
      if (error.status === 401) {
        ...
        try {
          ...
          if (data.accessToken) {
            ...
            setErrorMessage('Sorry, there was an error refreshing your access token. Please try again.');
          }
        } catch (error) {
          setErrorMessage('Sorry, there was an error refreshing your access token. Please try again.');
        }
      } else if (error.status === 403) {
        setErrorMessage("Sorry, This account doesn't seem to have Spotify Premium. Thank Spotify!")
        setSuccessMessage('')
      } else {
      ...
      <div className="container mt-4">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            ...
            <input type="text" className="form-control" id="songTitle" value={songTitle} onChange={handleSongTitleChange} required />
          </div>
          <div className="mb-3">
            <label htmlFor="songArtist" className="form-label">Song Artist:</label>
            <input type="text" className="form-control" id="songArtist" value={songArtist} onChange={handleSongArtistChange} required />
          </div>
          <div className="mb-3">
            <label htmlFor="songComments" className="form-label">Comments:</label>
            <textarea className="form-control" id="songComments" value={songComments} onChange={handleSongCommentsChange}></textarea>
          </div>
